...
- The bridge contract provides `ExtractionPlan`, `ExtractionPlanEntry`, collision policies, job events, terminal summaries, and job polling. The checked-in generated bindings follow the resolved sibling bridge; regenerate them from the selected `ZMANAGER_DIR` whenever the bridge revision changes. The temporary `.cache/zmanager` contract remains available for pinned compatibility reproduction.
